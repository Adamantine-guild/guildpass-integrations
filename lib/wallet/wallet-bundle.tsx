...
/**
 * lib/wallet/wallet-bundle.tsx
 *
 * HEAVY WALLET BUNDLE — dynamically imported by providers.tsx via next/dynamic.
 *
 * This module contains all wagmi / viem / @wagmi/connectors imports and the
 * entire SIWE auth provider.  By keeping it behind a dynamic import() with
 * ssr:false, the initial JS payload sent to public/unauthenticated pages is
 * *much* smaller — the wallet libraries (~200 kB gzipped) only load when a
 * user actually initiates a wallet connection.
 *
 * ── Module-level re-exports ─────────────────────────────────────────────────
 *
 * Components that need the SIWE auth context (useSiweAuth, SiweAuthContext)
 * import from '@/lib/wallet/siwe-context' — that module is lightweight and
 * never pulls in wagmi.  Only the layout's <RootProviders> touches this file.
 */
